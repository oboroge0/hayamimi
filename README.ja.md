...
| 翻訳 | `--translate en,zh,ko,es`で日本語行をリアルタイム翻訳（en=FuguMT、zh/ko/es=M2M-100） |
...
| `--translate [LANGS]` | オフ、`en` | 日本語行をカンマ区切りの言語（`en`/`zh`/`ko`/`es`）に翻訳 |
...
`--translate zh,ko,es`で使うM2M-100（MIT）には影響しません。
