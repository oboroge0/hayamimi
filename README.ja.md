...
- **翻訳は小型モデルなりの品質です。** とくに中国語・韓国語への翻訳は数値や金額を
  ...
  数字が大事な用途では先に見てください。
- **息継ぎなしで話される複数の文は、先頭の文が失われることがあります。** オフライン
  認識器は内部に無音のないVADセグメントを1発話として扱ってしまうため、hayamimiは
  0.35秒以上の内部無音で分割してこれを回避していますが、それより短い間隔で続けて
  話された文はまとめて1発話としてデコードされ、最後の文以外が落ちることがあります
  （head-dropout調査で見つかったclip-324系のケース）。
