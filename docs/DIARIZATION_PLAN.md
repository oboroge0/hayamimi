...
| 7（本ラウンド、§17） | セッション終了時グローバル再クラスタリング（§16の手法をライブ出力非改変で後乗せ） | simpleder平均+22.5pt悪化、5会議全て悪化、閾値非依存 | **不採用** |

**採用に至った本番動作変更は、この7ラウンドを通じて実質ゼロ**
（採用済みなのは評価スクリプトの機能や診断手法であって、`realtime_transcribe.py`
のデフォルト挙動そのものを動かしたものではない）。baseline
（pyannote 18.2% / simpleder 14.1%、§16実測）は Round 1〜7の
どの実験にも更新されていない。
...
- overlap floor（§16）: AMI 5会議平均で参照発話時間の約8.2%
  ...
  動かない。
- REMAP誤り（§14〜§15）: 清書グループのローカルクラスタを
  グローバルcentroidへ貪欲最近傍でremapする現行方式に起因する
  confusionは、remap側のノブ（joint割当、provisional除外）をどう
  動かしても悪化にしかならなかった。§15の分析どおり、LOCAL誤り
  （ローカルdiarizer自体の過分割）と独立ではなく、remap側を締めると
  LOCAL側に誤りが押し出されるトレードオフが観測されている。
- ファイル全体一括クラスタリングの利得（§16、-3.0pt）は実在するが、
  リアルタイム字幕という制約下でそのまま使える形になっていない。
  本ラウンド（§17）はこれをライブ出力非改変のまま回収しようとして
  失敗しており、次に狙うなら別のアーキテクチャが要る。
...
1. **overlap出力のUX**（§16で据え置き）: DER以前に「同時に2人の行を
   ...
   できない。
2. **セッション末尾ラベル訂正のUX**（本ラウンド§17で新たに明示化）:
   `Refiner.run_global_recluster()`は診断ログを1行printするだけで、
   コンソール・SSE・トランスクリプトのどれも書き換えない。仮に将来
   「対象漏れ」を解消した再クラスタリング手法が採用基準を満たしても、
   それを実際に使うにはコンソール出力・SSE配信・トランスクリプトファイル
   それぞれに「後から表示済みの行のラベルを訂正する」というUXが要る
   ——§9のcentroid merge（`merge_history()`）が抱えていたのと同種の、
   未解決のまま繰り越している問題。
