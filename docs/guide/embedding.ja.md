...
発話区間の途中ではなく、確定済み区間もすべて処理されたタイミングまで遅延され、
そこで作り直されます。待機中の変更は項目ごとに合成されます。たとえば
`threshold`の変更後に`min_silence`だけ変更しても、両方が保持されます。
同じ項目への再指定は最後の明示値が優先され、空または全項目がnullのVAD更新は
何も変更しません。再構築が成功するまで`GET /config`は現在有効な設定を返します。
再構築後も区間の絶対サンプル位置は維持されるため、音声履歴と清書処理が参照する
音声位置はずれません。
