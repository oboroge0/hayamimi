...
def _with_line_bound_translations(html: str) -> str:
    ...
    replacements = (
        ...
        (
            "      feed.appendChild(card); lastCard = card; stick(feed);\n",
            "      feed.appendChild(card); lastCard = card;\n"
            "      if (ev.line_id) cardsById.set(ev.line_id, card);\n"
            "      stick(feed);\n",
        ),
        (
            "      if (!lastCard) return;\n"
            "      const tr = document.createElement(\"div\"); tr.className = \"trans-line\";\n",
            "      const sourceCard = (ev.line_id && cardsById.get(ev.line_id)) || lastCard;\n"
    ...
    for old, new in replacements:
        if old not in html:
            raise RuntimeError("dashboard translation binding anchor is missing")
        html = html.replace(old, new, 1)
    return html


DASHBOARD_HTML = _with_line_bound_translations(DASHBOARD_HTML)
...
class SubtitleServer:
    ...
    def final(self, text: str, lang: str = "", speaker: str = "",
              latency_ms: float | None = None, tier: str = "", line_id: str = ""):
